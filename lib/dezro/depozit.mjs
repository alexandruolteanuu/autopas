...
export function creeazaDepozit({ url, key }) {
  ...
  // Bara de la finalul adresei se taie AICI, o singură dată.
  //
  // DE CE (constatat 7 septembrie 2026): variabila de mediu de pe Vercel se
  // termină cu „/", iar `${url}/storage/v1/...` a ieșit cu două bare. Adresele
  // funcționează — Supabase le normalizează, verificat cu HTTP 200 pe ambele
  // forme — dar au ajuns așa în `products.poze` la 8.855 din 8.964 de piese,
  // adică și în feed-urile de reclame.
  //
  // Rândurile vechi NU se rescriu, și e o decizie, nu lene: `dezro_anunturi.poze_trimise`
  // ține adresele NOASTRE ca text și le compară literal ca să știe ce poză e nouă.
  // O rescriere în masă ar face fiecare poză să pară nouă și ar reurca ~20.000 de
  // fișiere la dez.ro, pe un API care poate ține 90 de secunde per cerere.
  ...
  return {
     ...
     * degeaba se adună.
     *
     * `filtre` e un fragment PostgREST în plus, pentru rulările în care se
     * publică o parte anume din catalog („doar piesele care au deja cod OEM",
     * cât timp completarea codurilor încă merge). Nu schimbă niciodată regulile
     * de eligibilitate — doar restrânge mulțimea parcursă.
     *
     * Filtrele de bază sunt ACELEAȘI cu cele din view-ul `dezro_stare_piese`: dacă
     ...
     */
    async pieseEligibile(dupaId, cate, filtre = "") {
      return cere(
        `products?publicat=eq.true&stoc=gt.0&id=gt.${dupaId}${filtre ? "&" + filtre : ""}` +
    ...
    /** Ultimele anunțuri, cu numele piesei, pentru lista din panou.
     *  `product_id` e cheia primară, deci ordonarea pe el e stabilă. */
    async anunturiRecente(cate = 25) {
      const randuri = await cere(
        `dezro_anunturi?select=product_id,ad_id,url,status,aprobat,eroare,trimis_la` +
        `&order=trimis_la.desc.nullslast,product_id.desc&limit=${cate}`,
      );
      if (!randuri?.length) return [];
      const ids = randuri.map((r) => r.product_id).join(",");
      const piese = await cere(`products?id=in.(${ids})&select=id,nume,cod_intern`);
      const dupaId = new Map(piese.map((p) => [p.id, p]));
      return randuri.map((r) => ({
        ...r,
        nume: dupaId.get(r.product_id)?.nume ?? `#${r.product_id}`,
        cod_intern: dupaId.get(r.product_id)?.cod_intern ?? null,
      }));
    },
     ...
     */
    async numaraRanduri(tabela, coloana, filtre = {}) {
      // `select=` cere o COLOANĂ REALĂ. `select=1` pare o scurtătură inofensivă
      // (SQL acceptă `select 1`), dar PostgREST îl citește ca pe numele unei
      // coloane și răspunde 400 „column dezro_catalog.1 does not exist".
      // Cum apelantul prinde eroarea și pune 0, ecranul scria „Încă neadus"
      // peste un catalog adus complet — o cifră greșită care arată exact ca o
      // stare goală adevărată.
      const q2 = Object.entries(filtre).map(([k, v]) => `${k}=${v}`).join("&");
      const r = await fetch(`${url}/rest/v1/${tabela}?select=${coloana}${q2 ? "&" + q2 : ""}`, {
        headers: { ...h, Prefer: "count=exact", Range: "0-0", "Range-Unit": "items" },
      });
    ...
    // ---------- coada de sincronizare automată (migrarea 39) ----------
    /**
     * Rândurile de lucru, cele mai vechi întâi.
     *
     * `incercari` mărginește reîncercarea: după `maxIncercari` eșecuri rândul
     * rămâne în coadă, cu eroarea la vedere, dar nu mai e luat. O piesă cu o
     * categorie nemapată la ei n-are cum să se repare singură, iar reîncercarea
     * la infinit ar ține coada plină și ar ascunde restul.
     */
    async coadaCiteste(cate = 25, maxIncercari = 5) {
      return cere(
        `dezro_coada?incercari=lt.${maxIncercari}&select=id,product_id,ad_id,motiv,incercari&order=id&limit=${cate}`,
      );
    },

    /** Rândurile făcute se ȘTERG. Jurnalul a ce s-a trimis e `dezro_anunturi`;
     *  o coadă care își ține și istoricul ar crește la nesfârșit degeaba. */
    async coadaSterge(ids) {
      if (!ids?.length) return;
      for (let i = 0; i < ids.length; i += 200) {
        await cere(`dezro_coada?id=in.(${ids.slice(i, i + 200).join(",")})`, { method: "DELETE" });
      }
    },

    async coadaEsec(id, eroare, incercariVechi = 0) {
      await cere(`dezro_coada?id=eq.${Number(id)}`, {
        method: "PATCH",
        body: JSON.stringify({ incercari: incercariVechi + 1, eroare: String(eroare).slice(0, 300) }),
      });
    },

    async coadaStare() {
      const d = await cere("dezro_coada_stare?select=*");
      return d?.[0] ?? { total: 0, de_facut: 0, blocate: 0, de_sters: 0, cel_mai_vechi: null, o_eroare: null };
    },

    /**
     * Piesele DINTR-O LISTĂ care mai sunt eligibile acum.
     *
     * Se cer cu aceleași filtre ca `pieseEligibile` (publicată, cu stoc), fiindcă
     * răspunsul la „ce e de făcut cu piesa asta" e chiar diferența: e în listă =>
     * se trimite sau se actualizează; lipsește => i se retrage anunțul. Între
     * momentul în care triggerul a scris rândul și acum, piesa se poate fi
     * schimbat încă o dată — de aceea starea se citește ACUM, nu se ia din coadă.
     */
    async pieseDupaIduri(ids) {
      const out = [];
      for (let i = 0; i < ids.length; i += 200) {
        const lot = ids.slice(i, i + 200).join(",");
        out.push(...(await this.pieseEligibile(0, 200, `id=in.(${lot})`)));
      }
      return out;
    },

    /** Trezirea rutei, prin funcțiile din migrarea 39. `deblocheaza` pune ora
     *  ultimei treziri în trecut, ca următorul trigger să bată la ușă pe loc —
     *  se cheamă la finalul fiecărui lot, ÎNAINTE de recitirea cozii. */
    async deblocheazaTrezirea() {
      await cere("rpc/dezro_deblocheaza", { method: "POST", body: "{}" });
    },
    async trezesteAcum() {
      await cere("rpc/dezro_trezeste", { method: "POST", body: JSON.stringify({ fortat: true }) });
    },
