...
async function publicaPiesa({ sesiune, depozit, piesa, anunt, context, pana, timeout }) {
  ...
  // Pozele se compară DOAR cu un anunț care încă există la ei.
  //
  // DEFECT REPARAT la 14 septembrie 2026: o piesă vândută își pierdea anunțul
  // (status „retras", șters la ei cu tot cu poze), dar `poze_trimise` rămânea
  // lista vechiului anunț. La anularea comenzii piesa revenea în stoc, se
  // crea un anunț NOU, iar diferența se calcula față de lista veche — toate
  // pozele păreau „deja trimise", deci anunțul pleca fără nicio poză. Găsite
  // în producție: 5 anunțuri recreate fără poze, deși piesele aveau.
  const anuntViu = !!anunt?.ad_id && anunt.status !== "retras";
  const poze = anuntViu
    ? diferentaPoze(piesa.poze, anunt.poze_trimise, anunt.poze_dezro)
    : diferentaPoze(piesa.poze, [], []);
...
export async function lotPublicare({
  ...
  for (const piesa of piese) {
    ...
    try {
      ...
      await depozit.scrieAnunt(piesa.id, {
        // Un anunț deja ȘTERS la ei rămâne „retras" chiar dacă recrearea a
        // picat: cu „eroare" și `ad_id`-ul vechi, încercarea următoare ar
        // actualiza un anunț care nu mai există, în loc să creeze unul nou.
        status: vechi?.status === "retras" ? "retras" : "eroare",
...
// ------------------------------------------------------------
// 6b. UN LOT DIN COADA AUTOMATĂ (migrarea 39)
//
// DIFERENȚA FAȚĂ DE `lotPublicare` + `lotRetragere`
// Acelea parcurg TOT catalogul cu un cursor, ca să afle ce s-a schimbat: bune la
// prima publicare și la o resincronizare completă, prea scumpe pentru „s-a
// vândut o piesă acum trei secunde". Aici lista vine gata făcută, din
// `dezro_coada`, scrisă de triggerul din bază în aceeași tranzacție cu
// modificarea.
//
// PENTRU FIECARE RÂND, ce e de făcut se decide ACUM, din starea piesei de acum:
//   · piesa e publicată și are stoc  -> se trimite sau se actualizează, prin
//     EXACT același drum ca publicarea mare (`publicaPiesa`);
//   · piesa există dar nu mai e eligibilă (vândută, depublicată) -> i se retrage
//     anunțul;
//   · rândul are doar `ad_id` (piesa a fost ȘTEARSĂ din bază) -> se șterge
//     anunțul la ei. E singurul caz în care informația nu mai poate fi citită din
//     `products`, de aceea triggerul o copiază în coadă înainte de ștergere.
//
// Nu se decide nimic din `motiv`-ul scris de trigger: între trigger și procesare
// piesa se mai poate schimba o dată, iar o coadă care ține minte „era de
// publicat" ar publica o piesă vândută între timp.
// ------------------------------------------------------------
export async function lotCoada({
  depozit, sesiune, context, randuri,
  limitaMs = LIMITA_LOT_MS, bugetMs = BUGET_MS, timeout = undefined,
}) {
  const pana = acum() + limitaMs;
  const limita = acum() + bugetMs;
  const rez = {
    procesate: 0, publicate: 0, actualizate: 0, neschimbate: 0, sarite: 0,
    retrase: 0, poze: 0, erori: [], motive: {}, facute: [], oprit: null,
  };
  if (!randuri?.length) return rez;

  const idsPiese = randuri.filter((r) => r.product_id).map((r) => r.product_id);
  // Două citiri pentru tot lotul, nu două pe rând.
  const [eligibile, anunturi] = await Promise.all([
    idsPiese.length ? depozit.pieseDupaIduri(idsPiese) : Promise.resolve([]),
    idsPiese.length ? depozit.citesteAnunturiPentru(idsPiese) : Promise.resolve(new Map()),
  ]);
  const dupaId = new Map(eligibile.map((p) => [p.id, p]));

  for (const rand of randuri) {
    if (acum() > limita) break;
    try {
      // ---------- piesă ștearsă din bază: anunț orfan la ei ----------
      if (!rand.product_id && rand.ad_id) {
        await stergeAnuntTolerant(sesiune, rand.ad_id, { pana, timeout });
        rez.retrase++;
        rez.facute.push(rand.id);
        rez.procesate++;
        continue;
      }

      const piesa = dupaId.get(rand.product_id);
      if (piesa) {
        const r = await publicaPiesa({
          sesiune, depozit, piesa, anunt: anunturi.get(piesa.id) ?? null, context, pana, timeout,
        });
        ...
        // Nu mai e eligibilă. Dacă avea anunț activ, se stinge; dacă n-avea,
        // rândul se închide fără nicio cerere către ei — cazul obișnuit al unei
        // piese nepublicate care se editează.
        const a = anunturi.get(rand.product_id);
        if (a?.ad_id && a.status === "activ") {
          await stergeAnuntTolerant(sesiune, a.ad_id, { pana, timeout });
          await depozit.scrieAnunt(rand.product_id, {
            status: "retras", retras_la: new Date().toISOString(), eroare: null,
          });
          rez.retrase++;
        } else {
      ...
      const mesaj = e?.message ?? String(e);
      rez.erori.push({ coada_id: rand.id, id: rand.product_id ?? null, incercari: rand.incercari ?? 0, eroare: mesaj.slice(0, 300) });
      // Aceeași regulă ca la publicarea mare: rândul NU se închide și amprenta
      // rămâne cea veche. Un timeout nu înseamnă că n-a intrat nimic la ei, iar
      // retrimiterea aceleiași valori e inofensivă.
      if (rand.product_id) {
        await depozit.scrieAnunt(rand.product_id, {
          // Vezi nota din `lotPublicare`: un anunț retras nu devine „eroare".
          status: anunturi.get(rand.product_id)?.status === "retras" ? "retras" : "eroare",
          eroare: mesaj.slice(0, 300),
          incercari: (anunturi.get(rand.product_id)?.incercari ?? 0) + 1,
        }).catch(() => null);
      }
      if (e instanceof EroareDezro && (e.stare === 401 || e.stare === 403 || e.stare === 429)) {
        rez.oprit = mesaj;
...
/** Ștergerea unui anunț, cu 404 tratat ca reușită: dacă nu mai există la ei
 *  (l-a șters un admin de-al lor, sau noi înainte), rezultatul dorit e deja
 *  obținut. Aceeași regulă ca în `lotRetragere`, scoasă aici ca să nu existe
 *  două variante ale ei. */
async function stergeAnuntTolerant(sesiune, adId, opt) {
  try {
    await stergeAnunt(sesiune, adId, opt);
  } catch (e) {
    if (e instanceof EroareDezro && e.stare === 404) return;
    throw e;
  }
}

/**
 * Plasa de 20%, pentru coada automată.
 *
 * `PRAG_RETRAGERE` apăra publicarea mare de o depublicare în masă. Coada
 * automată are nevoie de ACEEAȘI plasă și cu atât mai mult: acolo omul apasă un
 * buton și citește un avertisment, aici nu apasă nimeni nimic. Un
 * `update products set publicat = false` dat din greșeală ar pune 8.800 de
 * rânduri în coadă, iar la ei ștergerea unui anunț NU se poate desface prin API.
 *
 * Se numără două lucruri, amândouă în bază:
 *   · anunțurile active care nu mai au piesă eligibilă (`dezro_de_retras`);
 *   · rândurile de coadă ale pieselor ȘTERSE (`dezro_coada_stare.de_sters`),
 *     care nu mai apar în primul număr fiindcă rândul lor din `dezro_anunturi`
 *     a plecat odată cu piesa.
 */
export async function pragCoada({ depozit }) {
  const [prag, coada] = await Promise.all([depozit.pragRetragere(), depozit.coadaStare()]);
  const deSters = Number(coada?.de_sters) || 0;
  const total = prag.deRetras + deSters;
  const procent = prag.active ? total / prag.active : 0;
  return {
    active: prag.active, deRetras: prag.deRetras, deSters, total, procent,
    depasit: prag.active > 0 && procent > PRAG_RETRAGERE,
  };
}
