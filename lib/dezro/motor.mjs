...
// 4. O SINGURĂ PIESĂ
// ------------------------------------------------------------
async function publicaPiesa({ sesiune, depozit, piesa, anunt, context, pana }) {
  ...
  if (!anunt?.ad_id || anunt.status === "retras") {
    const raspuns = await creeazaAnunt(sesiune, c.campuri, fisiere, { pana });
  ...
  for (const p of poze.deSters) {
    await stergePoza(sesiune, anunt.ad_id, p.id, { pana });
  }
  const doarCampuri = anunt.amprenta !== c.amprenta;
  const raspuns = await actualizeazaAnunt(
    sesiune, anunt.ad_id, doarCampuri ? c.campuri : {}, fisiere, { pana },
  );
 ...
 */
export async function lotPublicare({ cfg, depozit, sesiune, job, context, opresteDupaUna = false }) {
  const pana = acum() + LIMITA_LOT_MS;
  const limita = acum() + BUGET_MS;
  const rez = {
  ...
  const piese = await depozit.pieseEligibile(rez.pozitie, LOT_PIESE);
  ...
  for (const piesa of piese) {
    ...
    try {
      const r = await publicaPiesa({
        sesiune, depozit, piesa, anunt: anunturi.get(piesa.id) ?? null, context, pana,
      });
...
// ------------------------------------------------------------
export async function lotRetragere({ depozit, sesiune, job }) {
  const pana = acum() + LIMITA_LOT_MS;
  const limita = acum() + BUGET_MS;
  ...
  for (const a of randuri) {
    ...
    try {
      await stergeAnunt(sesiune, a.ad_id, { pana });
