...
import { aniDinTitlu } from "./extragere.mjs";
...
// Denumiri care NU sunt modele distincte, ci alt fel de a numi unul pe care îl
// avem deja. Cheia e „slugul mărcii|model normalizat", fiindcă multe din ele sunt
// prea generice ca să fie lăsate globale: „t5" înseamnă Transporter la Volkswagen,
// dar ar putea însemna orice la altcineva.
//
// Se adaugă aici doar cazuri VERIFICATE pe date, nu presupuneri:
//
//   · „Caddy Life" / „Caddy Maxi" sunt versiuni de echipare ale lui Caddy, nu
//     modele separate (verificat 25 august 2026).
//   · „T4"…„T7" sunt Transporterul. Sursa scrie pe ACEEAȘI piesă toate formele —
//     „Volkswagen T5", „Volkswagen TRANSPORTER" și „Volkswagen Transporter T5" —
//     iar fără alias primele două ar fi creat modele duplicate lângă
//     „Transporter T5", pe care îl avem de la început (verificat 28 august 2026,
//     37 de piese).
//   · „A8 4N" e codul intern al generației D5, iar „Fiesta 8" e a doua convenție
//     de numerotare pentru aceeași mașină ca „Fiesta 7" — vânzătorii scriu ambele
//     numere pentru același an (verificat 28 august 2026). Rândurile duplicate au
//     fost unificate de migrarea 26; fără aliasurile astea, primul import le-ar
//     recrea.
const ALIAS_MODELE = {
  "vw|caddy life": "caddy",
  "vw|caddy maxi": "caddy",
  "vw|t4": "transporter t4",
  "vw|t5": "transporter t5",
  "vw|t6": "transporter t6",
  "vw|t7": "transporter t7",
  "audi|a8 4n": "a8 d5",
  "ford|fiesta 8": "fiesta 7",
};
 ...
 * unul sub altul, deci diferența se vede. Se atinge DOAR prima literă; restul
 * scrierii vine de la sursă („CX-5", „S-cross").
 *
 * EXCEPȚIA: numele care încep cu una-două litere mici urmate de cifră (cu sau
 * fără cratimă între ele) NU se ating. Așa își scriu producătorii o familie întreagă de modele — Hyundai
 * „i20"/„i30"/„ix35", Toyota „bZ4X", Peugeot „e-208", BMW „i3" — iar aia e și
 * denumirea pe care o caută clientul. Denumirea oficială a producătorului bate
 * consecvența noastră internă (decizie 28 august 2026, după ce regula a scris
 * „I20" pe 49 de piese). „Jumpy" nu intră aici: „ju" nu e urmat de cifră, deci
 * primește majuscula — și e corect, Citroën chiar așa îl scrie.
 */
export const numeModelNou = (s) => {
  const t = (s ?? "").trim();
  if (/^[a-z]{1,2}-?\d/.test(t)) return t;
...
export function despartCompat(text, brands) {
  ...
  const alias = model ? ALIAS_MODELE[`${gasit.brand.slug}|${model}`] : null;
...
/**
 * Sugestia pentru formularul „Adaugă piesă": marca, modelul și anii citiți din
 * titlul scris de operator.
 *
 * NU e o potrivire automată care scrie ceva în bază: rezultatul completează
 * câmpurile goale din formular, iar omul le vede și le poate schimba înainte de
 * „Publică piesa". Diferența contează — CLAUDE.md interzice deducerea din titlu
 * atunci când nimeni nu confirmă (la „VW Golf 6 1.6 TDI", „6" din „1.6" lega
 * piese de Golf 7).
 *
 * Folosește EXACT regulile importului, în ordinea asta:
 *   1. `modelDinTitlu` — marca și modelul cu generație scrise în titlu
 *      („Skoda Octavia 2");
 *   2. marca din titlu + primele cuvinte de după ea, trecute prin `potriveste`,
 *      ca o linie de compatibilitate — de acolo vine alegerea generației după
 *      ani („Skoda Octavia 2008" -> Octavia 2), cu aceeași regulă strict /
 *      prin suprapunere ca la import;
 *   3. fără marcă în titlu („Far Octavia 2008"): marca se deduce din numele de
 *      model, doar dacă UNA SINGURĂ are un model cu numele ăla, iar apoi se
 *      reia pasul 2.
 *
 * Capacitatea cilindrică se scoate înainte („1.6", „2.0"), tocmai ca „6" din
 * „1.6" să nu mai poată fi citit drept generație.
 */
export function sugestieDinTitlu(titlu, taxonomie) {
  const { brands, models } = taxonomie;
  const ani = aniDinTitlu(titlu ?? "");
  const rez = {
    brand: null, model: null,
    an_min: ani[0] ?? null, an_max: ani[ani.length - 1] ?? null,
    // Cuvintele din titlu care descriu MAȘINA (marcă, model). Restul descriu
    // piesa și sunt cele după care se caută categoria.
    cuvinteMasina: [],
  };
  const curat = String(titlu ?? "").replace(/\b\d[.,]\d\b/g, " ");
  const tokeni = normalizeaza(curat).split(" ").filter(Boolean);
  const baza = (m) => normalizeaza(m.nume.replace(/\(.*$/, ""));
  const pozitia = (secventa) => {
    const k = secventa.split(" ");
    for (let i = 0; i + k.length <= tokeni.length; i++)
      if (k.every((c, j) => tokeni[i + j] === c)) return i;
    return -1;
  };
  const gata = (brand, model, cuvinte) => {
    rez.brand = brand; rez.model = model;
    rez.cuvinteMasina = [...new Set(cuvinte)];
    return rez;
  };

  // 1. marca și modelul cu generație, scrise ca atare.
  //    Capcana Peugeot: „2008" e și model, și an. În „Peugeot 207 2007 2008"
  //    `modelDinTitlu` alege „2008" (numele mai lung câștigă). Un nume de model
  //    care arată a an și chiar e printre anii titlului nu se ia aici; un
  //    Peugeot 2008 adevărat e găsit la pasul 2, după cuvântul de lângă marcă.
  //    Măsurat la 14 septembrie 2026: 20 din cele 64 de sugestii greșite.
  const faraAni = models.filter((m) => !ani.includes(Number(baza(m))));
  const direct = modelDinTitlu(curat, { brands, models: faraAni });
  if (direct) {
    const chei = cheiMarca(direct.brand).map(normalizeaza);
    return gata(direct.brand, direct.model, [...chei.flatMap((k) => k.split(" ")), ...baza(direct.model).split(" ")]);
  }

  // 2. marca din titlu + familia de model, generația din ani
  const dinFamilie = (brand, start, cheieMarca) => {
    const dupa = tokeni.slice(start);
    for (let k = Math.min(3, dupa.length); k >= 1; k--) {
      const linie = `${brand.nume} ${dupa.slice(0, k).join(" ")}`;
      const p = potriveste({ titlu: curat, compat: [linie], an_min: rez.an_min, an_max: rez.an_max }, taxonomie);
      const model = p.model_id ? models.find((m) => m.id === p.model_id) : null;
      if (model) return gata(brand, model, [...cheieMarca.split(" "), ...dupa.slice(0, k), ...baza(model).split(" ")]);
    }
    return null;
  };
  const marci = [];
  for (const b of brands) for (const k of cheiMarca(b).map(normalizeaza)) {
    const i = k ? pozitia(k) : -1;
    if (i >= 0) { marci.push({ brand: b, dupa: i + k.split(" ").length, cheie: k }); break; }
  }
  if (new Set(marci.map((x) => x.brand.id)).size === 1) {
    const { brand, dupa, cheie } = marci[0];
    return dinFamilie(brand, dupa, cheie) ?? gata(brand, null, cheie.split(" "));
  }
  if (marci.length) return rez; // două mărci în titlu: nu alegem noi

  // 3. fără marcă: familia de model trebuie să fie a UNEI singure mărci
  const familii = new Map(); // brand_id -> poziția primului cuvânt din familie
  for (const m of models) {
    const primul = baza(m).split(" ")[0];
    if (!primul || primul.length < 4 || !/[a-z]/.test(primul)) continue;
    const i = tokeni.indexOf(primul);
    if (i >= 0 && !familii.has(m.brand_id)) familii.set(m.brand_id, i);
  }
  if (familii.size !== 1) return rez;
  const [[brandId, i]] = [...familii];
  const brand = brands.find((b) => b.id === brandId);
  return brand ? (dinFamilie(brand, i, "") ?? rez) : rez;
}
