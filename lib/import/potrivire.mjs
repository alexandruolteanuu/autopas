...
function potrivesteOLinie(text, ext, taxonomie) {
  ...
  if (exact.length > 1) { out.note.push(`model ambiguu: „${mCompat}"`); return out; }
...
export function potriveste(ext, taxonomie) {
  ...
  rez.de_creat_modele = potriviri
    .filter((p) => !p.model && p.brand && p.normalizat && p.brut && !p.generatiiCandidate)
  ...
  if (marci.length && !marci.some((p) => [p.brand.nume, p.brand.slug].some((k) => t.includes(" " + normalizeaza(k) + " ")))) {
