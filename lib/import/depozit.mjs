...
export function creeazaDepozit({ url, key }) {
  ...
  return {
    ...
    async asiguraModel({ slug, nume, brand_id }) {
      const gasit = await cere(`models?slug=eq.${encodeURIComponent(slug)}&select=id,nume,slug,brand_id`);
      if (gasit?.[0]) return gasit[0];
      try {
      ...
      const dinNou = await cere(`models?slug=eq.${encodeURIComponent(slug)}&select=id,nume,slug,brand_id`);
