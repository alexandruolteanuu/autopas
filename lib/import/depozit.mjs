...
export function creeazaDepozit({ url, key }) {
   ...
   *
   *  Fiecare apelant trebuie să pună `order=` pe o coloană UNICĂ (de regulă
   *  `id`). Fără ordine stabilă, Postgres n-are nicio obligație s-o păstreze
   *  între pagini: același rând poate veni de două ori, iar altul deloc — și
   *  „altul deloc" înseamnă, la depublicare, o piesă stinsă pe nedrept. */
  ...
  return {
    ...
    async citesteTaxonomia() {
      const [brands, models, categories] = await Promise.all([
        ...
        tot("models?select=id,nume,brand_id,an_start,an_final&order=id"),
        tot("categories?select=id,nume,slug,parent_id,art,ordine&order=id"),
      ]);
    ...
    async citesteToateDeLaSursa(sursa) {
      return tot(`products?sursa=eq.${encodeURIComponent(sursa)}&select=id,sursa_id,nume,pret_lei,editat_manual,publicat,sursa_activ&order=id`);
    },
    ...
    async citesteTotPentruCompletare(sursa) {
      return tot(`products?sursa=eq.${encodeURIComponent(sursa)}&select=id,nume,sursa_url,categorie_id,subcategorie_id,model_ids,compat,editat_manual&order=id`);
    },

    /** Piesele cu ce trebuie ca să li se completeze codul (`oem`) și să li se
     *  repare descrierea tăiată (`scripts/completeaza-oem.mjs`). `stare_nota` e
     *  coloana grea a tabelei, de aceea nu intră în celelalte citiri complete. */
    async citesteTotPentruOem(sursa) {
      return tot(`products?sursa=eq.${encodeURIComponent(sursa)}&select=id,cod_intern,sursa_url,oem,stare_nota,editat_manual&order=id`);
    },
