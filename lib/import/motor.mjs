...
import { potriveste, potrivesteCategoria } from "./potrivire.mjs";
import { construiesteRand, patchLaReimport } from "./rand.mjs";
...
export async function proceseazaRanduri({
  ...
  for (const r of randuri) {
    ...
    if (!a.ok) {
      // Eroare FATALĂ = mediul de rulare nu poate ajunge la sursă (vezi
      // EROARE_FARA_CURL). N-are sens să marcăm mii de rânduri ca eșuate și nici
      // să le reluăm: nimic nu se schimbă până nu se schimbă mediul. Oprim lotul
      // cu motivul la vedere, exact ca la canar.
      if (a.fatal) {
        rez.pagini--;
        rez.canar.total--;
        rez.oprit = "mediu";
    ...
    // Categoria: `null` la numărul de piese, fiindcă în flux nu se știe totalul
    // pe categorie-sursă. Nimic nu se creează automat; slug-ul se numără mai jos
    // și apare în raportul final.
    let cat = { categorie_id: null, subcategorie_id: null, note: [] };
    if (taxonomie && ext.categorie_sursa) {
      cat = potrivesteCategoria(ext.categorie_sursa, taxonomie.categories, null, null);
