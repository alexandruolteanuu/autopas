...
import { potriveste, potrivesteCategoria, modelDinTitlu, normalizeaza, numeModelNou, sugestieDinTitlu } from "./potrivire.mjs";
import { construiesteRand, construiesteCiorna, patchLaReimport, slugifica } from "./rand.mjs";
...
// Cât are voie să dureze aducerea TUTUROR pozelor unei piese. Fiecare poză are
// deja timeout-ul ei (20s), dar o piesă cu opt poze lente le-ar înmulți și ar
// depăși limita funcției. Iar cum un rând neterminat nu avansează poziția, importul
// s-ar bloca exact pe ea, la nesfârșit — același tipar ca defectul de 504 din
// 25 august 2026. Pozele rămase se notează; piesa intră cu câte a apucat.
export const BUGET_POZE_MS = 15000;

// INVARIANTUL care ține un lot sub limita funcției serverless (60s):
//
//   BUGET_MS + TIMEOUT_PAGINA_MS + BUGET_POZE_MS  <=  55s
//   15        + 20                + 15             =   50s
//
// Bugetul se verifică ÎNAINTE de fiecare rând, deci în cel mai rău caz lotul mai
// începe un rând chiar la limită, iar acel rând poate costa o pagină lentă plus
// toate pozele ei. Cine schimbă una din cele trei valori trebuie să refacă suma —
// `scripts/verifica-import.mjs` o verifică.
export const LIMITA_LOT_MS = 55000;
 ...
 */
export function planifica(randuriCsv, existente, { pragDepublicare = PRAG_DEPUBLICARE, faraPagini = false } = {}) {
  ...
  return {
    ...
    // Fără pagini nu există pauză politicoasă: totul vine din CSV, deci ~instant.
    minuteEstimate: faraPagini ? 0 : Math.round((noi.length * (PAUZA_MS + 1000)) / 60000),
  };
}

/** Convertește și urcă o poză de piesă importată, cu numele de fișier al importului.
 *  Comună importului clasic și preluării din browser (`app/api/preia-anunt`). */
export async function urcaPozaImport(depozit, sursaId, i, brut) {
  ...
  return { url: await depozit.urcaPoza(cale, date, tip), cale, octeti: date.length };
}
...
async function aduPozele(depozit, sursaId, urls, bugetMs = BUGET_POZE_MS) {
  const salvate = [], erori = [];
  const start = Date.now();
  let octeti = 0;
  for (let i = 0; i < urls.length; i++) {
    if (i > 0 && Date.now() - start > bugetMs) {
      erori.push(`pozele ${i + 1}–${urls.length} sărite: aducerea lor a depășit ${bugetMs / 1000}s`);
      break;
    }
    try {
      const brut = await aducePoza(urls[i]);
      const p = await urcaPozaImport(depozit, sursaId, i, brut);
      salvate.push(p.url);
      octeti += p.octeti;
    } catch (e) {
...
export async function asiguraModelul(depozit, taxonomie, ext, pot) {
  ...
  // `brand_pentru_creare` și `model_compat*` sunt puse de `potriveste` doar pentru
  // o linie de compatibilitate pe care titlul o confirmă — vezi acolo de ce.
  const nume = pot.model_compat_brut;
  if (!pot.brand_pentru_creare || !nume || !pot.model_compat) return null;
  ...
  const model = await depozit.asiguraModel({
    ...
    nume: numeModelNou(nume),
    brand_id: marca.id,
  });
  if (!taxonomie.models.some((m) => m.id === model.id)) taxonomie.models.push(model);
  return { model_id: model.id, model: model.nume, brand_id: marca.id, marca: marca.nume,
           nota: `model nou, creat din compatibilitatea sursei și confirmat de titlu: „${nume}"` };
}

/**
 * Compatibilitățile ÎN PLUS: mașinile pe care sursa le enumeră și pe care noi nu
 * le avem în tabelă, la o piesă care are deja cel puțin un model recunoscut.
 *
 * Aici nu se mai cere confirmarea titlului. Titlul numește mașina de pe care s-a
 * demontat piesa; celelalte linii sunt exact informația pentru care există lista —
 * „se potrivește și la". A le cere să apară în titlu ar însemna să le aruncăm pe
 * toate. Modelele se creează, iar piesa devine găsibilă și de clienții acelora.
 */
export async function asiguraModeleleInPlus(depozit, taxonomie, pot) {
  const adaugate = [], note = [];
  for (const cerut of pot.de_creat_modele ?? []) {
    ...
    try {
      const model = await depozit.asiguraModel({
        slug: `${marca.slug}-${slugifica(cerut.nume)}`,
        nume: numeModelNou(cerut.nume),
        brand_id: marca.id,
      });
      if (!taxonomie.models.some((m) => m.id === model.id)) taxonomie.models.push(model);
      if (!adaugate.includes(model.id)) adaugate.push(model.id);
      note.push(`compatibilitate în plus: ${marca.nume} ${model.nume}`);
    } catch (e) { note.push(`model „${cerut.nume}": ${e.message}`); }
  }
  return { adaugate, note };
}

/**
 * O PIESĂ DIN PAGINA EI: extragere, potrivire, model, categorie, poze, rândul final.
 *
 * Scoasă din bucla lui `proceseazaRanduri` la 15 septembrie 2026, ca s-o poată
 * folosi și preluarea din browser (`app/api/preia-anunt`): acolo pagina nu o cerem
 * noi de la pieseauto.ro, ci vine din browserul operatorului, iar pozele sosesc
 * deja urcate. Regulile rămân UNA singură — o piesă preluată cu butonul iese
 * identică cu una din importul clasic.
 *
 * `aduPoze(urls)` întoarce `{ salvate, erori, octeti }`; nu se cheamă în modul uscat
 * și nici când pagina n-are poze. Nu scrie piesa în bază — doar o construiește.
 */
export async function piesaDinPagina({ depozit, taxonomie, html, urlFinal, feed, uscat = false, aduPoze }) {
  let modeleCreate = 0, categoriiCreate = 0;
  const canonic = urlCanonic(html, urlFinal);
  const ext = extrage(html, canonic);
    ...
    try {
      if (!pot.model_ids?.length) {
        ...
        if (m) {
          ...
          pot.model_ids = [m.model_id];
          revizuire.push(m.nota);
          if (m.nota.startsWith("model nou")) modeleCreate++;
        }
      } else {
        const { adaugate, note } = await asiguraModeleleInPlus(depozit, taxonomie, pot);
        pot.model_ids = [...pot.model_ids, ...adaugate.filter((id) => !pot.model_ids.includes(id))];
        revizuire.push(...note);
        modeleCreate += adaugate.length;
      }
  ...
  // întoarce oricum, ca operatorul să vadă în raport ce a intrat și de unde.
  ...
  const rand = construiesteRand(
    { ...ext, ...pot, ...cat, feed, sursa_id: feed.ID, sursa_url: canonic, revizuire },
    taxonomie?.categories, poze.salvate,
  );
  rand.sursa_sincronizat_la = new Date().toISOString();

  return { rand, ext, pot, cat, poze, revizuire, canonic,
           categorieSursa: taxonomie ? ext.categorie_sursa : null, modeleCreate, categoriiCreate };
}
 ...
 *
 * @param {{
 *   depozit: any, randuri: any[], taxonomie?: any, uscat?: boolean, faraPagini?: boolean,
...
export async function proceseazaRanduri({
  ...
  // SINCRONIZAREA FĂRĂ PAGINI (15 septembrie 2026): piesele noi NU se descarcă de
  // la pieseauto.ro (ne blochează), ci intră ca ciorne nepublicate, din CSV.
  // Vezi `construiesteCiorna` din rand.mjs.
  faraPagini = false,
  ...
  // DOAR rândurile pe care lotul ăsta le poate atinge. Cine cheamă motorul îi dă
  // toată coada feed-ului (`randuri.slice(job.procesate)`), iar bucla se oprea
  // oricum la `maxRanduri` — dar căutarea de mai jos întreba baza despre TOATE.
  // La un feed de 8.500 asta însemna 85 de cereri REST (3,4 secunde măsurate)
  // înainte de a procesa 4 piese, repetate la fiecare lot: ~2 ore aruncate pe
  // un import întreg. Felia face răspunsul identic, cu 5 cereri în loc de 85.
  const deLucru = randuri.slice(0, maxRanduri);

  // Ce există deja, aflat dintr-o singură cerere pentru toată felia.
  const existente = await depozit.citesteExistente(SURSA, deLucru.map((r) => r.ID));
  ...
  for (const r of deLucru) {
    ...
    // ---------- piesă nouă, fără pagină: ciornă din CSV ----------
    if (faraPagini) {
      const sug = taxonomie ? sugestieDinTitlu(r.Titlu, taxonomie) : null;
      const rand = construiesteCiorna(r, sug);
      rand.sursa_sincronizat_la = new Date().toISOString();
      if (!uscat) {
      ...
      rez.procesate++;
      laProgres?.({ rand: r, tip: "ciorna", rez });
      continue;
    }
    ...
    const { rand, ext, pot, poze, revizuire, categorieSursa, modeleCreate, categoriiCreate } =
      await piesaDinPagina({
        depozit, taxonomie, uscat, html: a.html, urlFinal: a.urlFinal, feed: r,
        aduPoze: (urls) => aduPozele(depozit, r.ID, urls),
      });
    if (!ext.poze.length) rez.canar.faraPoze++;
    if (categorieSursa) rez.categoriiSursa[categorieSursa] = (rez.categoriiSursa[categorieSursa] ?? 0) + 1;
    rez.modeleCreate += modeleCreate;
    rez.categoriiCreate += categoriiCreate;
