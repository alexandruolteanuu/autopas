...
/** ID-ul anunțului din adresa lui pieseauto.ro — „…-1463955097.html" sau
 *  „produs-1463955097.html". `null` pentru orice adresă din afara pieseauto.ro.
 *  E cheia care leagă o pagină deschisă în browser de `products.sursa_id`
 *  (preluarea din browser: `app/admin/preia-anunt`, `app/api/preia-anunt`). */
export function idAnuntDinAdresa(adresa) {
  try {
    const u = new URL(adresa);
    if (!/(^|\.)pieseauto\.ro$/.test(u.hostname)) return null;
    return u.pathname.match(/(?:produs-|-)(\d+)\.html$/)?.[1] ?? null;
  } catch { return null; }
}
...
export function codOem(descriere) {
  ...
  /** CRATIMA CU SPAȚII e ambiguă, iar cele două înțelesuri ale ei se despart
   *  curat (numărat pe cele 101 rânduri „COD:" rămase necitite la 7 septembrie
   *  2026): ori ține codul împreună — BMW îl scrie rupt, „1241- 8570238 - 03" —
   *  ori începe vorbirea vânzătorului, „1K0501204F - dreapta spate".
   *
   *  Ce urmează după cratimă hotărăște: un fragment SCURT, dintr-o bucată, care
   *  nu e un cuvânt (deci are cifre, ori e o singură literă — sufixele „- A",
   *  „- B") se lipește înapoi la cod; orice altceva e vorbire, și tot ce urmează
   *  se taie. „- AF" și „- ej" sunt sufixe reale pe care le pierdem așa, dar nu
   *  se pot deosebi de „- NU are elementul cromat"; codul de bază rămâne corect,
   *  iar un câmp puțin mai scurt e mai bun decât unul cu vorbire în el.
   *
   *  Se despart DOAR cratimele cu spațiu — „bj3m-502s1-aa" rămâne întreg. */
  const lipesteCratime = (s) => {
    const bucati = s.split(/\s+-+\s*|-+\s+/);
    let out = bucati[0];
    for (const b of bucati.slice(1)) {
      if (!/^[A-Za-z0-9.]{1,8}$/.test(b) || /^[A-Za-z]{2,}$/.test(b)) break;
      out += "-" + b;
    }
    return out;
  };
  ...
  const gasite = [];
  // „cod", apoi cel mult două cuvinte („cod injector"), apoi „:". Limita de
  // cuvânt e cerută DOAR la sfârșit — ea e cea care apără „Codul postal:". La
  // început n-are ce căuta: sursa lipește rândul de ce e înaintea lui, iar forma
  // cea mai deasă e chiar lipită de un an, „…2010COD: 96629590".
  for (const m of descriere.matchAll(/cod\b[^\n:]{0,20}:\s*([^\n]{0,120})/gi)) {
    // Taie fraza lipită de cod. Un cuvânt NU e cuvânt când urmează imediat o
    // cifră sau o cratimă — acolo e chiar codul: „BCJH-51040" și „BCKA-50221"
    // (Mazda) cădeau pe „patru majuscule la rând", „Bos1-50221" pe „majusculă
    // urmată de litere mici". Regula veche le tăia din primul caracter și lăsa
    // un șir gol, deci le pierdea pe toate, tăcut.
    const taiat = m[1].split(/[A-ZÎÂĂȘȚ][a-zîâășț]{2,}(?![0-9-])|[A-Z]{4,}(?![0-9-])/)[0];
    ...
    // Un al doilea „COD:" în același rând desparte la fel: sursa nu pune mereu
    // `<br>`, deci ies lipite — „13717800575COD: 13718510365".
    for (const bucata of taiat.split(/\s*[/;,]\s*|\s+(?:respectiv|si|sau|ori)\s+|\s*cod\b[^:]{0,20}:\s*/i)) {
      const c = lipesteCratime(bucata.replace(/\s+/g, " ").trim()).replace(/[.,;:)!?\-\s]+$/, "").trim();
      if (!c) continue;
