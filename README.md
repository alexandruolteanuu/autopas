  ...
  return (
    <button
      onClick={() => { if (inCos) { router.push("/cos"); return; }
        add({ id: p.id, slug: p.slug, nume: p.nume, pret: Number(p.pret_lei), art: p.art, oem: p.oem });
        router.push("/cos"); }}
      className={`inline-flex items-center justify-center gap-2 rounded-lg font-disp font-bold uppercase tracking-wide transition
