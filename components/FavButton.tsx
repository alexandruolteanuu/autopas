...
export default function FavButton({ id, variant = "card" }: { id: number; variant?: "card" | "line" }) {
  ...
  if (variant === "line")
    return (
      <button onClick={() => comuta(id)} aria-pressed={activ} title={eticheta}
        className={`inline-flex items-center gap-2 rounded-xl border-2 px-4 min-h-[44px] text-sm font-medium transition
          ${activ ? "border-accentChenar accentuat bg-accent/5" : "border-chenar text-text hover:border-accentChenar"}`}>
  ...
  return (
    <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); comuta(id); }}
      ...
      className={`shrink-0 w-11 h-11 rounded-full grid place-items-center border transition
        ${activ ? "bg-accent text-accentContrast border-accentChenar" : "border-chenar text-textSecundar accentuat-hover hover:border-accentChenar"}`}>
      <Inima plina={activ} />
    </button>
  );
}
